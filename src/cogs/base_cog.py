...
class base(commands.Cog):
    ...
    async def on_ready(self):
        ...
        if users is not None:
            ...
            user_default = users.load_defaults(directory='settings/database')
        ...
        if guilds is not None:
            ...
            guild_default = guilds.load_defaults(directory='settings/database')
    ...
    async def build_table(self,table_name,settings):
        ...
        testRow = await self.bot.db.execute("SELECT * FROM %s"%table_name)
        if testRow == None:
